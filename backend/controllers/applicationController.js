...
export const jobseekerGetAllApplication=catchAsyncError(async(req,res,next)=>{
    ...
    if(role==="Employer"){
        return next(new ErrorHandler(
            "Employer is not allowed to acces this resoursces!",
            400
        ))
    }
    const {_id}=req.user;
...
export const postAppliction=catchAsyncError(async(req,res,next)=>{
    const{role}=req.user;
    if(role==="Employer"){
        return next( new ErrorHandler(
            "Employer is not allowed to access this resoursces!",
            400
        ))
    }
